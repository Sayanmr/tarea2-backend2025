...
export class SocketHandler {
    ...
    initEvents() {
        this.iosocket.on("connection", (socket) => {
            ...
            socket.on("apply-discount", async (payload) => {
                const { productId } = payload;
                const updatedProduct = await this.applyDiscount(productId);
                if (updatedProduct) {
                    this.iosocket.emit("discount-applied", updatedProduct);
                }
            });
